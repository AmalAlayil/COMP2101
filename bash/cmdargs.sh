...
while [ $# -gt 0 ]; do
  ...
  # recognize the help option -h and the verbose mode -v
  case $1 in
      -h )
  # print help
      echo "-h for help"
  ...
  # print verbose
      echo "-v for verbose"
  # assign the test1 value to a variable
      test1=1
      ;;
  # recognize -d N for debug
      -d )
        case $2 in
          [1-6] )
          echo "-d for debug level $2"
  # assign test2 to variable
          test2=$2
          shift
          ;;
          *)
      ...
      ;;
      *)
  #display theverror message if user gave -d option without a single digit number
      error=$1
  ...
  shift
  # tell the user we have shifted things
  echo "Shifted command line, leaving $# things left to process."
  echo "--------------------------"
  # go back to the top of loop to see if anything is left to work on
done
echo "Done"
...
# tell the user if vebose is ON or OFF
...
# tell user if the dedug is ON Or OFF and the number is set to
