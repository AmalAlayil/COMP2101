...
interface=$(ifconfig | grep -w -o '^[^ ][^ ]*:' | tr -d :)

#Using the for loop to generate report for all interfaces except loopback by specifying a condition not to
for interface in $interface; do
  #condition given to not generate report for loopback interface
  if [ $interface = "lo" ] ; then
    continue
  fi

# Finding address and hostname for the interface being summarized
# we then assume that there is only one IPV4 address assigned to this interface
ipv4_address=$(ip a s $interface|awk -F '[/ ]+' '/inet /{print $3}')
ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')

# Identify the network number for this interface and name if it has one
